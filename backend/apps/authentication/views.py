...
from .serializers import (
    ...
    LoginSerializer, SocialMediaAccountSerializer, TeamMemberSerializer, UserRegistrationSerializer
)
...
def verify_email(request, token):
    try:
        ...
        if verification_token.is_used:
            return Response({'error': 'This verification link has already been used'}, 
                           status=status.HTTP_400_BAD_REQUEST)
        
        if verification_token.is_expired():
            return Response({'error': 'This verification link has expired'}, 
                           status=status.HTTP_400_BAD_REQUEST)
