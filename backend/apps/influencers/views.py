...
def influencer_dashboard(request):
    ...
    try:
        ...
        dashboard_data = {
            ...
            'recent_applications': CampaignApplicationSerializer(
                applications.order_by('-applied_at')[:5], many=True
            ).data,
            'available_campaigns': CampaignListSerializer(
                recent_campaigns, many=True
            ).data,
        }
...
def import_influencers(request):
    ...
    try:
        ...
        if source == 'csv':
            ...
            return Response({
                "message": f"Imported {created_count} influencers from CSV",
                "source": "csv",
            ...
            return Response({
                "message": f"Imported {created_count} influencers",
                "source": "manual",
                "total_imported": created_count
            })
        
        return Response(
            {"error": "Invalid source. Use 'csv' or 'manual'"}, 
    ...
    try:
        ...
        if influencer_id:
            ...
            for post in mentioned_posts:
                ...
                try:
                    post_analytics = AnalyticsData.objects.filter(
                        post=post,
            ...
            return Response({
                ...
                'summary': {
                    'total_mentions': len(mentioned_posts),
        ...
        return Response(
            {"error": "Failed to get influencer analytics"}, 
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
