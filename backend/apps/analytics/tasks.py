...
import logging
import random
from .models import AnalyticsData, PerformanceReport, CommentAnalytics, PlatformAverage, BestPerformingPost, AnalyticsInsight
...
def analyze_comment_sentiment(post_id):
    ...
    try:
        ...
        sentiment = random.choices(sentiments, weights=weights)[0]
        sentiment_score = random.uniform(0.5, 0.9) if sentiment == "positive" else (
            random.uniform(-0.9, -0.5) if sentiment == "negative" else random.uniform(-0.4, 0.4)
        )
...
def calculate_platform_averages(user_id, period_type='weekly'):
    ...
    try:
        ...
        all_analytics = AnalyticsData.objects.filter(
            user=user,
            date__gte=start_date,
            date__lte=end_date
        )
        ...
        overall_metrics['avg_engagement_rate'] = (total_engagement / overall_metrics['avg_reach'] * 100) if overall_metrics['avg_reach'] > 0 else 0
...
def generate_performance_report(user_id, report_type='weekly'):
    ...
    try:
        ...
        best_posts = BestPerformingPost.objects.filter(
            user=user,
...
def send_report_email(report_id):
    ...
    try:
        ...
        html_content = render_to_string('emails/performance_report.html', {
            'report': report,
            'user': report.user
        })
...
def identify_best_performing_posts(user_id, period_type='weekly'):
    ...
    try:
        ...
        for platform in platforms:
            ...
            posts_analytics = AnalyticsData.objects.filter(
                user=user,
                platform=platform,
                date__gte=start_date,
                date__lte=end_date,
            ...
            for metric_type in ['engagement_rate', 'reach', 'impressions', 'likes']:
                ...
                for post_analytics in posts_analytics:
                    ...
                    if metric_type == 'engagement_rate':
                        # Calculate engagement rate
                        likes = AnalyticsData.objects.filter(
                            post_id=post_id, metric_type='likes', date__gte=start_date
                        ).aggregate(total=models.Sum('value'))['total'] or 0
                        
                        shares = AnalyticsData.objects.filter(
                            post_id=post_id, metric_type='shares', date__gte=start_date
                        ).aggregate(total=models.Sum('value'))['total'] or 0
                        
                        comments = AnalyticsData.objects.filter(
                            post_id=post_id, metric_type='comments', date__gte=start_date
                        ).aggregate(total=models.Sum('value'))['total'] or 0
                        
                        reach = AnalyticsData.objects.filter(
                            post_id=post_id, metric_type='reach', date__gte=start_date
                        ).aggregate(total=models.Sum('value'))['total'] or 0
                        
                        engagement_rate = ((likes + shares + comments) / reach * 100) if reach > 0 else 0
                        metric_value = engagement_rate
                    else:
                        metric_value = AnalyticsData.objects.filter(
